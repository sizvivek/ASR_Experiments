...
# Construct a three-shot prompt using the first three examples
num_shots = 3
three_shot_prompt = ""
if 'few_shot_examples_full' in locals() and len(few_shot_examples_full) >= num_shots:
    few_shot_examples_to_use = few_shot_examples_full[:num_shots]
    three_shot_prompt = construct_few_shot_prompt(few_shot_examples_to_use)
    print(f"\n📝 Constructed {num_shots}-shot prompt:\n---\n{three_shot_prompt}\n---")
else:
     print(f"\n⚠️ Not enough examples defined for {num_shots}-shot prompting ({len(few_shot_examples_full)} examples available). Three-shot prompt will be empty.")
     three_shot_prompt = ""


# Initialize a list to store the results for three-shot prompting
three_shot_results = []
...
if test_files_to_process: # Check if there are files to process after API key check
    print(f"\n▶️ Starting three-shot ASR process for {len(test_files_to_process)} files...")
...
if test_files_to_process: # Check again if processing is still possible
    ...
    if test_files_to_process: # Final check before starting file loop
        ...
        for fname in test_files_to_process:
            ...
            # Run ASR (three-shot)
            print(f"✨ Running ASR on {fname} (Three-Shot)...")
            try:
                with open(save_path, "rb") as f:
                    response = client.audio.transcriptions.create(
                        model="gpt-4o-mini-transcribe",
                        file=f,
                        prompt=three_shot_prompt if three_shot_prompt else None # Pass the three-shot prompt if constructed
                    )
                pred_text = response.text.strip()
                print("✅ ASR transcription complete.")
                # print("Prediction:", pred_text) # Uncomment for more detailed logging
                ...
                # Store results
                three_shot_results.append({
                    "file": fname,
                    "ground_truth": gt_text,
                    "prediction": pred_text,
                    "WER": wer,
                    "prompt_type": "three-shot"
                })
            except Exception as e:
                print(f"❌ An unexpected error occurred during ASR or WER calculation for {fname}: {e}. Skipping.")
                continue

    print("\n🏁 Finished processing files for three-shot prompting.")

# ===============================
# 🔹 Save and Show Three-Shot Results
# ===============================
print("\n📝 Saving and showing three-shot results...")
if three_shot_results:
    df_three_shot = pd.DataFrame(three_shot_results).sort_values("WER")
    df_three_shot.to_csv("asr_three_shot_results.csv", index=False)

    print("\n✅ Three-shot transcription results saved to asr_three_shot_results.csv\n")
